...
/** The same inclusive end as {@link formatPeriodEndDate}, with a full date ("Aug 31, 2026") for surfaces that show one. */
export function formatPeriodEndFull(periodEnd: string | Date, locale: string = 'en-US', options?: Intl.DateTimeFormatOptions): string {
	const end = toInclusivePeriodEnd(periodEnd);
	if (isNaN(end.getTime())) return 'Invalid Date';
	return end.toLocaleDateString(locale, { year: 'numeric', month: 'short', day: '2-digit', ...options, timeZone: 'UTC' });
}
...
/** Format a billing period as "Jun 1, 2026 – Aug 31, 2026" for surfaces that show the full date. Same exclusive-end rule as {@link formatBillingPeriod}. */
export function formatBillingPeriodLong(periodStart: string, periodEnd: string): string {
	const start = new Date(periodStart);
	if (isNaN(start.getTime()) || isNaN(new Date(periodEnd).getTime())) return 'Invalid Date';
	const startText = start.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
	return `${startText} – ${formatPeriodEndFull(periodEnd, 'en-US', { day: 'numeric' })}`;
}
